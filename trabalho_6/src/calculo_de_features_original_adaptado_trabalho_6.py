...
def Compute_First_Order_Statistics_Features(image, Bins):
    ...
    image = image.astype(
        np.uint8
    )  # Convertendo para uint8 para garantir que os valores estejam na faixa de 0 a 255.
    image = image.flatten().astype(
        np.float64
    )  # Achata a imagem e converte para float64 para os cálculos.
...
if __name__ == "__main__":
    ...
    for arquivo in imagens:
        ...
        first_Oder_Statistics_features = Compute_First_Order_Statistics_Features(
            image, Bins
        )
